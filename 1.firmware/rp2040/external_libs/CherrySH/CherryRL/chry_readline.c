...
struct command_t{
  uint8_t length;
  char cmd[MAX_CMD_LENGTH]; //store maximum is 32 character
};
//add new function to manage cache
static void chry_readline_history_cache_store(chry_readline_t *rl)
{
  // history_buf = [len1 | command1][len2 | command2]....
  // history_buf will act like stack, last in first out
  if(rl->hist.in * sizeof(struct command_t) > rl->hist.size)
  {
    //TODO
    uint8_t num_of_removed_item = 1;
    memmove(rl->hist.pbuf, rl->hist.pbuf + num_of_removed_item * sizeof(struct command_t), rl->hist.size - num_of_removed_item * sizeof(struct command_t));
    rl->hist.in = rl->hist.in - num_of_removed_item;
  }
    
  struct command_t *cmd = (struct command_t *) (rl->hist.pbuf + rl->hist.in * sizeof(struct command_t));
  cmd->length = rl->ln.buff->size <= sizeof(cmd->cmd) ? rl->ln.buff->size : sizeof(cmd->cmd);
  memcpy(cmd->cmd, rl->ln.buff->pbuf, cmd->length);
...
static void chry_readline_history_cache_load(chry_readline_t *rl, uint16_t direction)
{
  if(direction){
    if(rl->hist.cache < (rl->hist.in-1)){
      rl->hist.cache++;
    }
  }else{
    if(rl->hist.cache > 0)
  ...
  struct command_t *cmd = (struct command_t *) (rl->hist.pbuf + rl->hist.cache * sizeof(struct command_t));
  rl->ln.buff->size = cmd->length;
  memcpy(rl->ln.buff->pbuf, cmd->cmd, cmd->length);
}
...
static void chry_readline_history_store(chry_readline_t *rl)
{
    if (rl->ln.buff->size > 0) {
        ...
        /*!< update cache index */
        //rl->hist.cache = rl->hist.in;
      chry_readline_history_cache_store(rl);
      //chry_readline_put(rl, rl->hist.pbuf, rl->hist.size, ); //just for test
    }
}
...
{
    chry_readline_history_cache_load(rl, index);
    rl->ln.curoff = rl->ln.buff->size;
    rl->hist.index = index;
    return chry_readline_edit_refresh(rl);
}
...
static int chry_readline_history_loadprev(chry_readline_t *rl)
{
    ...
    return chry_readline_history_load(rl, 0);
}
...
static int chry_readline_history_loadnext(chry_readline_t *rl)
{
    ...
    return chry_readline_history_load(rl, 1);
}
